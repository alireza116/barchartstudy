function wrappedBarChart(indexNumber, wrapThresh,threshPortion){
    ...
    var fileNames = ["PresidentialCandidates","gunDeathsByState","congressMisconduct"];
//    var index = 0;
    var fileName;
    var sortValue;
    var xValue;
...
//                return color(values[i]);
            ...
            .attr("d", valueline).on("mouseover",function(d){
...
//                    return color(values[i]);
        ...
        //y axis
        yAxis = svg.append("g")
            .call(d3.axisLeft(y).tickSize(-width)).style("font-size","0.8em");
        ...
        var helpAxisRange = svg.append("text")
            .attr("transform","translate("+(width + 10)+"," + helpTranslate+")")
...
//            .style("display", "none");
        ...
        focus.append("line")
            .attr("x1",0).attr("x2",0).attr("y1",height).attr("y2",0).attr("stroke","red").attr("stroke-width",strokeWidth).attr("stroke-opacity",0.5);
...
//            .text(ticks[xI]);
        focus
            .attr("transform","translate(" + x(tickPos[xI]) + "," + 0 + ")");
